...
{
    public async Task<ResultadoT<ReclutadorDto>> Handle(ActualizarReclutadorCommand solicitud, CancellationToken cancellationToken)
    {
        ...
        var reclutador = await repositorioReclutador.ObtenerByIdAsync(solicitud.ReclutadorId, cancellationToken);
        if (reclutador is null)
        {
            logger.LogError("No se encontró el reclutador con el ID {ReclutadorId}.", solicitud.ReclutadorId);
            return ResultadoT<ReclutadorDto>.Fallo(Error.NoEncontrado("404", "El reclutador no existe."));
        }

        reclutador.NombreEmpresa = solicitud.NombreEmpresa;
